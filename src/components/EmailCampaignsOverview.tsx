...
const EmailCampaignsOverview: React.FC<EmailCampaignsOverviewProps> = ({ onNavigate }) => {
  ...
  // Force email mode for this component
  const { campaigns, loading, error, refetch } = useCampaignData('email');
        ...
        {error ? (
          ...
          <div className="flex items-center justify-center py-12">
            <div className="flex items-center space-x-3">
              ...
              <span style={{ color: '#888888' }}>Loading email campaigns...</span>
            </div>
          </div>
        ) : campaigns.length === 0 ? (
          <div className="text-center py-12">
            ...
            <h3 className="text-xl font-semibold text-white mb-2">No Email Campaigns Found</h3>
            <p className="text-gray-400 mb-6">
              {filter === 'All' 
                ? 'Create your first email campaign to get started'
                : `No email campaigns with status "${filter}"`
              }
            </p>
            {filter === 'All' && (
              <button 
...
export default EmailCampaignsOverview;