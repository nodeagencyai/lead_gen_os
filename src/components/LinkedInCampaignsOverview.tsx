...
interface LinkedInCampaignsOverviewProps {
  onNavigate: (view: 'dashboard' | 'leadfinder' | 'campaigns' | 'leads' | 'integrations' | 'monitoring') => void;
}

const LinkedInCampaignsOverview: React.FC<LinkedInCampaignsOverviewProps> = ({ onNavigate }) => {
  ...
  // Force LinkedIn mode for this component
  const { campaigns, loading, error, refetch } = useCampaignData('linkedin');
  ...
  const handleExport = useCallback(() => {
    const csvContent = [
      ['Campaign Name', 'Status', 'Total Contacted', 'Response Rate', 'Connection Rate', 'Message Rate', 'Leads Ready', 'Messages Sent'].join(','),
      ...filteredCampaigns.map(campaign => 
        [
          campaign.name,
          campaign.status,
          campaign.totalContacted || 0,
          `${campaign.openRate || 0}%`, // Response rate for LinkedIn
          `${campaign.clickRate || 0}%`, // Connection rate for LinkedIn
          `${campaign.replyRate || 0}%`, // Message rate for LinkedIn
          campaign.leadsReady || 0,
          campaign.emailsSent || 0 // Messages sent for LinkedIn
        ].join(',')
      )
    ].join('\n');
    ...
    a.href = url;
    a.download = `linkedin-campaigns-${new Date().toISOString().split('T')[0]}.csv`;
    a.click();
        ...
        <div className="flex items-center justify-between mb-8">
          <div>
            ...
            <p className="text-base" style={{ color: '#ffffff' }}>
              Real-time LinkedIn campaign analytics from HeyReach
            </p>
          </div>
          <div className="flex items-center space-x-4">
            ...
            >
              <Plus size={16} />
              <span>New LinkedIn Campaign</span>
            </button>
          </div>
        </div>
        ...
        {error ? (
          ...
          <div className="flex items-center justify-center py-12">
            <div className="flex items-center space-x-3">
              ...
              <span style={{ color: '#888888' }}>Loading LinkedIn campaigns...</span>
            </div>
          </div>
        ) : campaigns.length === 0 ? (
          <div className="text-center py-12">
            ...
            <h3 className="text-xl font-semibold text-white mb-2">No LinkedIn Campaigns Found</h3>
            <p className="text-gray-400 mb-6">
              {filter === 'All' 
                ? 'Create your first LinkedIn campaign to get started'
                : `No LinkedIn campaigns with status "${filter}"`
              }
            </p>
            {filter === 'All' && (
              <button 
              ...
              >
                <Plus size={16} />
                <span>Create LinkedIn Campaign</span>
              </button>
            )}
          </div>
        ) : (
          ...
          {filteredCampaigns.map((campaign) => (
            ...
            >
              ...
              {/* Campaign Analytics - LinkedIn specific labels */}
              <div className="space-y-3 mb-6">
                ...
                <div className="grid grid-cols-2 gap-3">
                  ...
                  <div className="p-2 rounded" style={{ backgroundColor: '#0f0f0f', border: '1px solid #333333' }}>
                    ...
                    <div className="text-xs" style={{ color: '#888888' }}>Response Rate</div>
                  </div>
                </div>

                {/* Row 2: Connection & Message Stats */}
                <div className="grid grid-cols-2 gap-3">
                  <div className="p-2 rounded" style={{ backgroundColor: '#0f0f0f', border: '1px solid #333333' }}>
                    <div className="text-lg font-bold text-white">{campaign.clickRate}%</div>
                    <div className="text-xs" style={{ color: '#888888' }}>Connection Rate</div>
                  </div>
                  <div className="p-2 rounded" style={{ backgroundColor: '#0f0f0f', border: '1px solid #333333' }}>
                    <div className="text-lg font-bold text-white">{campaign.replyRate}%</div>
                    <div className="text-xs" style={{ color: '#888888' }}>Message Rate</div>
                  </div>
                </div>
                ...
                <div className="grid grid-cols-2 gap-3">
                  ...
                  <div className="p-2 rounded" style={{ backgroundColor: '#0f0f0f', border: '1px solid #333333' }}>
                    ...
                    <div className="text-xs" style={{ color: '#888888' }}>Messages Sent</div>
                  </div>
                </div>
              </div>
...
export default LinkedInCampaignsOverview;