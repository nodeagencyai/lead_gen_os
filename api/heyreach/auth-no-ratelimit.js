// Temporary test endpoint without rate limiting to diagnose issue
export default async function handler(req, res) {
  ...
  if (req.method === 'OPTIONS') {
    console.log('🔄 CORS preflight request handled for /api/heyreach/auth-no-ratelimit');
  ...
  try {
    ...
    console.log('🔄 Testing HeyReach API key via serverless function (no rate limit)...');
    ...
    console.log('✅ HeyReach authentication successful (no rate limit)');
    res.status(200).json({ 
      success: true,
    ...
    console.error('❌ HeyReach auth error (no rate limit):', error);
    res.status(500).json({ 
